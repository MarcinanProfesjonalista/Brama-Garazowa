...
void jazda() {
  ...
  // Reset AUTO STOP przy nowej akcji
  if (chceWGore || chceWDol) {
    autoStop = false;
  }
  ...
  if (!momentaryUp && !momentaryDown && !trybSET) {
    stopSilnik();
    return;
  }
  ...
  if (trybSET) {
    ...
    if (jedzie_w_gore && millis() - startCzasJazdy > czasGora) {
      autoStop = true;
      stopSilnik();
      trybSET = false;
      return;
    ...
    if (jedzie_w_dol && millis() - startCzasJazdy > czasDol) {
      autoStop = true;
      stopSilnik();
      trybSET = false;
      return;
