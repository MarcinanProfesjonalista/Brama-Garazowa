...
String pobierzStanDrzwi(
  ...
  if (momentaryUp && !krGorna) return "OTWIERANIE";
  if (momentaryUp && krGorna) return "KR Gorna AKT";
  if (momentaryDown && !krDolna) return "ZAMYKANIE";
  if (momentaryDown && krDolna) return "KR Dolna AKT";
  

  if (trybSET && jedzie_w_gore) return "AUTO OTWIERANIE";
  if (trybSET && jedzie_w_dol)  return "AUTO ZAMYKANIE";
...
void wyswietl_status_oled(
  ...
  display.print("OTW");
  display.print(BUTTON_UP_SET_PIN_wcisniety||PILOT_UP_Bi_SET_PIN_wcisniety ? "1 " : "0 ");

  display.print("ZAM");
  display.print(BUTTON_DOWN_SET_PIN_wcisniety||PILOT_DOWN_Ai_SET_PIN_wcisniety ? "1" : "0");
...
void obsluga_wyswietlania() {
  ...
  bool set_UP =
    BUTTON_UP_SET_PIN_wcisniety ||
    PILOT_UP_Bi_SET_PIN_wcisniety;

  bool set_DOWN =
    BUTTON_DOWN_SET_PIN_wcisniety ||
